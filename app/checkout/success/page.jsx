              ...
              {order ? (
                <>
                      ...
                      {Array.isArray(order.items) && order.items.length > 0 ? (
                        order.items.map((it) => (
                          <tr key={it.id}>
                            ...
                            <td>{it.nombre}</td>
                            <td className="text-end">${Number(it.precio || 0).toLocaleString("es-CL")}</td>
                            <td className="text-center">{Number(it.cantidad || 0)}</td>
                            <td className="text-end">${((Number(it.precio || 0) * Number(it.cantidad || 0)) || 0).toLocaleString("es-CL")}</td>
                          </tr>
                        ))
                      ) : (
                        <tr>
                          <td colSpan={5} className="text-center text-muted">No hay items en la orden</td>
                        </tr>
                      )}
                    </tbody>
                  </Table>
                  ...
                  <div className="d-flex gap-2 mt-4">
                    ...
                    <Button variant="outline-primary" onClick={() => router.push("/")} >Volver al inicio</Button>
                  </div>
                </>
              ) : (
